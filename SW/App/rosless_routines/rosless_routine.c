...
char increment(double *s0, double *s1, double *s2, double *s3, double s0_target, double s1_target, double s2_target, double s3_target) {
	char changed = 0;
	if (*s0 < s0_target) {
		*s0 += SPEED_PER_T;
...
int main(int argc, char** argv){
    ...
    while (current != NULL) {
		...
		if (current->command.entity == ARM) {
			while (increment(&s0, &s1, &s2, &s3, current->command.vector.values[0], current->command.vector.values[1], current->command.vector.values[2], s3)) {
			...
			while (increment(&s0, &s1, &s2, &s3, s0, s1, s2, current->command.vector.values[0])) {
				duties[0] = angle_to_duty(s0);
				duties[1] = angle_to_duty(s1);
				duties[2] = angle_to_duty(s2);
				duties[3] = angle_to_duty(s3);
